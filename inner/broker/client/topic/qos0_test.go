...
type mockPublishWriter struct {
	buffer *bytes.Buffer
}

func newMockPublishWriter() *mockPublishWriter {
	return &mockPublishWriter{
		buffer: bytes.NewBuffer(nil),
	}
}

func (m *mockPublishWriter) Bytes() []byte {
	return m.buffer.Bytes()
}

func (m *mockPublishWriter) WritePacket(packet packets.Packet) {
	packet.WriteTo(m.buffer)
}
